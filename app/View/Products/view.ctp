...
<!--                 <dt><?php echo __('Price'); ?></dt>
        ...
        <!-- CRITERIA VALUES -->
        <div class="range_types">
            <h4>Range Values</h4>
            <?php 
            if (!empty($products['RangeValue'])):
                foreach ($range_types as $id => $name):
                ?>
                <div class="ib va-top">
                    <h5><?php echo $name ?></h5>
                    <?php
                    foreach ($products['RangeValue'] as $rangeValue):
                        if($rangeValue['range_type_id'] == $id):
                            echo $this->Form->input('',array('label'=>$rangeValue['name'],'type'=>'checkbox','multiple'=>'checkbox','checked','disabled'=>'disabled','class'=>'ib'));
                        endif;
                    endforeach;
                    ?>
                    <br>
